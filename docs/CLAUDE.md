...
- `src/yeast-agent` - 830 lines (expanded from 616)
- `src/yeast` - Enhanced with consolidate/audit support
- `docs/PHASE-2/FEATURES.md` - Comprehensive Phase 2 design & features
- `docs/PHASE-2/TEST_GUIDE.md` - Complete Phase 2 test suite
- Updated `docs/CLAUDE.md`, `CHANGELOG.md`
...
- `src/yeast` - Main CLI wrapper (root-level wrapper delegates to this)
- `src/yeast-agent` - Agent loop implementation (runs on apollo.local)
- `src/setup-apollo.sh` - Deployment script
- `docs/README.md` - Comprehensive user guide
- `CHANGELOG.md` - Detailed release notes (at project root)
...
```
ai-yeast/
├── yeast                          (Root-level wrapper → src/yeast)
...
│   ├── yeast                      (Main CLI script)
│   ├── yeast-agent                (Python agent for apollo.local)
│   └── setup-apollo.sh            (Deployment script)
│
├── docs/                          (All documentation)
│   ├── README.md                  (Documentation index)
│   ├── CLAUDE.md                  (This file - developer guide)
│   ├── GEMINI.md                  (Gemini layer documentation)
│   ├── INSTRUCTIONS.md            (Original Phase 1 spec)
│   ├── TESTING.md                 (General testing guide)
│   └── PHASE-2/
│       ├── FEATURES.md            (Phase 2 design & features)
│       └── TEST_GUIDE.md          (Phase 2 comprehensive test suite)
│
├── plans/                         (Project planning)
│   ├── phase-2-remaining-work.md  (Known issues & fixes)
│   ├── phase-3-roadmap.md         (Future direction)
│   └── README.md                  (Plans index)
│
├── data/                          (Local memory downloads)
│   └── downloads/                 (Backup files from apollo)
│
└── .archives/                     (Old backups)
    └── yeast-backup-*/            (Historical snapshots)
```

**Key Design**:
- Root level is clean (only wrapper, config, docs link, changelog)
- Source code isolated in `src/`
- Documentation organized in `docs/`
- Memory data in `data/` for easy cleanup/backup
- Old backups archived in `.archives/`
